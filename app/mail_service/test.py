...
def test_send_email():
    ...
    """ 
    result = send_email(os.getenv("TEST_EMAIL"), os.getenv("TEST_RECEIVER"), os.getenv("TEST_PASSWORD"), test_html_content, "Test Subject", email_type=EmailTypes.HTML)
    if result["status"] != EmailSendStatus.SUCCESS:
        raise Exception("Failed to send email", result.get("e"))
